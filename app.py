    ...
    try:
        ...
        if interview_type == 'general':
            ...
            initial_prompt = (
                ...
                "10. 応募者が感謝の言葉を述べたら、「どういたしまして。面接にご参加いただきありがとうございました。結果については後日連絡いたします。」と伝えて面接を終了してください。\n"
                "11. 応募者が「面接を終わりにしたい」と伝えたら、「かしこまりました。本日の面接は終了です。面接にご参加いただきありがとうございました。結果については後日連絡いたします。」と伝えて面接を終了してください。\n"
                f"最初の質問は「本日は障害者雇用の面接にお越しいただきありがとうございます。まず、ご自身の障害特性と、業務遂行上必要となる合理的配慮について教えていただけますでしょうか？」です。" # 障害者雇用向けに調整
            )
        else:
...
def get_feedback():
    try:
        ...
        feedback_prompt = (
            ...
            "良かった点: 面接での良い回答や態度、印象的な発言、適切な対応など、評価できる点を具体的に挙げてください。\n"
            "改善点: より良い回答ができた点、改善すべき態度や発言、不足していた要素などを指摘し、具体的な返答例を一つ含めてください。返答例は「返答例: [具体的な返答例]」の形式で記述してください。\n"
            "総合評価: 面接全体を通しての印象、強みと弱みのバランス、今後の改善に向けたアドバイスなどを総合的に評価してください。\n"
            "各カテゴリ（良かった点、改善点、総合評価）は必ず新しい行から始めてください。\n"
            "最後に、100点満点での評価点数を「評価点数: [点数]/100」の形式で出力してください。点数は整数でお願いします。\n"
            "【重要】応募者の返答内容が極端に短い、意味をなさない、または「あ」や「はい」などの単語のみ、もしくは無回答の場合は、厳しめの評価・具体的な指摘・点数を大きく下げてください。\n"
            "【特に重要】音声入力が全くない場合（会話履歴がAIの質問のみ、またはユーザーの返答が一切ない場合）は、評価点数を必ず0点にしてください。\n"
            "【最重要】面接と全く関係のない発言、支離滅裂な発言、暴言、意味のない言葉、文脈の通っていない発言の場合は、評価点数を必ず0点とし、フィードバックも一切出力しないでください。\n"
            "面接履歴:\n"
...
def synthesize_speech(text, gender):
    ...
    try:
        ...
        # 性別に基づいて音声を選択
        if gender == "MALE":
            voice_name = "ja-JP-Wavenet-D" # 男性声の例
