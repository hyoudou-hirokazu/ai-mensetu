...
def get_feedback():
    ...
    try:
        ...
        # フィードバックのプロンプトをさらに厳密な形式で指示
        feedback_prompt = (
            f"あなたは面接練習アプリの面接官です。以下の面接履歴全体を評価し、面接における「あなた」へのフィードバックを以下の厳密な形式で生成してください。\n"
            "箇条書きや特殊文字（例: *）は使用しないでください。\n"
            "各項目は必ず新しい行から始めてください。\n\n"
            "**形式:**\n"
            "良かった点: [良かった点を簡潔に記述]\n"
            "改善点: [改善点を簡潔に記述]\n"
            "返答例: [具体的な返答例を一つ記述。例がない場合は「なし」と記述]\n"
            "総合評価: [総合評価を簡潔に記述]\n"
            "評価点数: [点数]/100 (点数は整数)\n\n"
            "面接履歴:\n"
